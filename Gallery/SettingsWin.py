...
class Settings(tkinter.Toplevel):
    def __init__(self):        
        ...
        frameUp.pack(fill=tkinter.BOTH, expand=True)

        frameB = MyFrame(self)
        frameB.pack()

        LeftMenu(frameUp).pack(side=tkinter.LEFT, padx=(0, 10))

        General(frameUp).pack(fill=tkinter.BOTH, expand=True)
        Expert(frameUp)
...
class LeftMenu(MyFrame):
    ...
    def Change(self, frameForget=tkinter.Frame, framePack=tkinter.Frame, 
        ...
        framePack.pack(fill=tkinter.BOTH, expand=True)
...
class BelowMenu(MyFrame):
    def __init__(self, master):
        ...
        btnSave.pack(side=tkinter.LEFT, padx=10)

        btnCancel = MyButton(self, text='Отмена')
        btnCancel.Cmd(lambda e: self.cancel())
        btnCancel.pack(side=tkinter.LEFT)
...
class General(MyFrame):
    def __init__(self, master):
        ...
        descrLabel.pack(pady=(0, 10), anchor=tkinter.W)
        ...
        descrLabel2.pack(pady=(0, 10), anchor=tkinter.W)
        ...
        createdBy.configure(
            text=made, justify=tkinter.LEFT)
        createdBy.pack(anchor=tkinter.W)
...
class Expert(tkmacosx.SFrame):
    def __init__(self, master):
        ...
        for key, value in data.items():
            ...
            desrc.pack(anchor=tkinter.W, pady=(0, 10))
            ...
            btnCopy.pack(side=tkinter.LEFT, padx=(0, 10))
            ...
            btnPaste.pack(side=tkinter.RIGHT, padx=(0, 10))

            sep = Separator(self, orient='horizontal')
            sep.pack(padx=40, pady=20, fill=tkinter.X)
        
        for ins, descr in zip(labelsInserts, descriptions):
            ins.configure(text=descr, justify=tkinter.LEFT, wraplength=340)
